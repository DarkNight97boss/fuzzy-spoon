...
ob_start();
	...
	if($do == "dele"){
		$check2 = $db->query("UPDATE users SET rank = '1' WHERE id = '$key'");
		$_SESSION['HK_GOOD_RETURN'] = "Utente srankato con successo";
		header("LOCATION: ". HK ."/ranks.php");		
	}
		
	if(isset($_POST['dairank'])){
		if(empty($_POST['ruoloutente']) || empty($_POST['rankutente']) || empty($_POST['name'])){
			...
			$result = mysql_query("UPDATE users SET rank = '".$user->filtertext($_POST['rankutente'])."', rank_lijst = '".$user->filtertext($_POST['ruoloutente'])."' WHERE username = '".$user->filtertext($_POST['name'])."'") or die(mysql_error());
...
<?php if ($user->Get('rank') > 6) : ?>
		<div id="hk-body">
			<div id="body">
				...
				<div id="box">
					<div class="title yellow"><b>Staff di Hubix Hotel</b></div>
...
display: inline-block;">
						 ...
						 <table class="striped">
							...
							<tbody>
								<hr>
								<?php	
									$get_ranks = $db->query("SELECT * FROM users WHERE rank > '1' AND rank < '9' ORDER BY rank ASC");
								...
								<tr>
									...
									<td class="right"><center><a href="<?php echo HK; ?>/ranks.php?do=dele&key=<?php echo $row['id']; ?>"><img src="<?php echo CDN; ?>/images/del.gif"></a></center></td>
									</tr>
								<?php } ?>
							</tbody>
							<tfoot></tfoot>
				...
				<div id="box">
					...
					<div class="content-box" id="addban">	
						<form action="<?php echo HK; ?>/ranks.php" method="post" name="dairank" id="dairank">
