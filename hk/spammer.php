...
ob_start();
	...
	if($do == "dele"){
		$check2 = $db->query("UPDATE users SET spammer = '0' WHERE id = '$key'");
		$_SESSION['HK_GOOD_RETURN'] = "Utente srankato con successo";
		header("LOCATION: ". HK ."/spammer.php");		
	}
	...
	if(isset($_POST['dairank'])){
		if(empty($_POST['ruoloutente']) || empty($_POST['name'])){
			...
			$result = mysql_query("UPDATE users SET spammer = '1', rank_lijst = '".$user->filtertext($_POST['ruoloutente'])."' WHERE username = '".$user->filtertext($_POST['name'])."'") or die(mysql_error());
...
<?php if ($user->Get('rank') > 6) : ?>
		<div id="hk-body">
			<div id="body">
				...
				<div id="box">
					<div class="title yellow"><b>Spammer di Hubix Hotel</b></div>
...
display: inline-block;">
						 ...
						 <table class="striped">
							...
							<tbody>
								<hr>
								<?php	
									$get_ranks = $db->query("SELECT * FROM users WHERE spammer = '1' ORDER BY id ASC");
								...
								<tr>
									...
									<td class="right"><center><a href="<?php echo HK; ?>/spammer.php?do=dele&key=<?php echo $row['id']; ?>"><img src="<?php echo CDN; ?>/images/del.gif"></a></center></td>
									</tr>
								<?php } ?>
							</tbody>
							<tfoot></tfoot>
				...
				<div id="box">
					...
					<div class="content-box" id="addban">	
						<form action="<?php echo HK; ?>/spammer.php" method="post" name="dairank" id="dairank">
