function initializeExternalInterfaces () {
	"use strict";
	console.log("Starting Edited Habbo API... Nearly recommended for Development usages.");
	window.HabboFlashClient.init ( document.getElementById ( "flash-container" ) );
}
!function () {
	"use strict";
	var n = "*";
	window.MainApp = { postMessage: function ( e ) {window.parent.postMessage ( e, n )} }
} (), function () {
	"use strict";
	var n = !1;
	...
	}, window.FlashExternalInterface.track = function ( n, e, a ) {window.HabboFlashClient.started = !0, window.HabboTracking.track ( n, e, a )}, window.FlashExternalInterface.updateFigure = function ( n ) {
		window.MainApp.postMessage ( {
			call: "update-figure",
			target: n
		} )
	...
	}, window.FlashExternalInterface.openAvatars = function () {window.FlashExternalInterface.openPage ( "/settings/avatars" )}
} (), function () {
	"use strict";
	function n ( n ) {
		if (n.data) {
			var o = n.data;
			switch (console.log ( "Received event, with call: " + o.call ), o.call) {
				case"open-link":
					e ( o.target );
					break;
				case"open-room":
	...
	window.HabboPageTransformer = {
		translate: function ( a ) {
			...
			return a
		}, transformHabblet: function ( n, e ) {return "/" + n}
	}
} (), function () {
	"use strict";
	function n ( n, e ) {
		$.ajax ( {
			...
			dataType: "json",
			type: "GET"
		} ).done ( function ( n ) {e ( null, n )} ).fail ( function ( n ) {e ( n, null )} )
	}

	window.HabboShopApi = {}, window.HabboShopApi.checkOffer = function ( e ) {n ( "/shopapi/checkoffer/", e )}
} (), function () {
	"use strict";
	var n = function ( n, e, a ) {"console" in window && "log" in console && console.log ( "action = [" + n + "], label = [" + e + "], value = [" + a + "]" )};
	window.HabboTracking = {
		track: function ( e, a, o ) {n ( e, a, o ), "clientload" === e && window.HabboTracking.gaTrack ( e, a )},
		gaTrack: function ( n, e ) {window._gaq && window._gaq.push ( [ "_trackEvent", n, e ] )}
	}
} (), function () {
	"use strict";
	function n ( n, e, a ) {
		...
		} ).done ( function ( n ) {a ( null, n )} ).fail ( function ( n ) {a ( n, null )} )
	}

	window.HabboWebApi = {}, window.HabboWebApi.checkName = function ( e, a ) {
		//n ( "/api/newuser/name/check", { name: e }, a ) // Disabled NUX Step
	}, window.HabboWebApi.claimName = function ( e, a ) {
		//n ( "/api/newuser/name/select", { name: e }, a ) // Disabled NUX Step
	}, window.HabboWebApi.saveFigure = function ( e, a, o ) {
		//n ( "/api/user/look/save", { figure: e, gender: a }, o ) // Disabled NUX Step
	}, window.HabboWebApi.selectRoom = function ( e, a ) {
		//n ( "/api/newuser/room/select", { roomIndex: e }, a ) // Disabled NUX Step
	}, window.HabboWebApi.logCrash = function ( e, a ) {
		console.log("Handled Client Crash: " + e) // Changed to Console Logging
	}, window.HabboWebApi.logError = function ( e, a ) {
		console.log("Handled Client Error: " + e); // Changed to Console Logging
	}, window.HabboWebApi.logLoginStep = function ( e, a, o ) {
		console.log("Handled Login Step: " + a); // Changed to Console Logging
	}
} (), function () {
	"use strict";
	window.NewUserReception = {}, window.NewUserReception.checkName = function ( n ) {console.log ( 'Checking name: "' + n + '"...' ), window.HabboWebApi.checkName ( n, function ( e, a ) {return e ? (console.error ( "Checking name failed!" ), void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameFailed ( n )) : void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameResponse ( n, a.code, a.validationResult || [], a.suggestions )} )}, window.NewUserReception.chooseRoom = function ( n ) {console.log ( 'Choosing room: "' + n + '"...' ), window.HabboWebApi.selectRoom ( n, function ( n ) {return n ? (console.error ( "Choosing room failed!" ), void window.HabboFlashClient.flashInterface.newUserReceptionChooseRoomFailed ()) : void window.HabboFlashClient.flashInterface.newUserReceptionChooseRoomResponse ()} )}, window.NewUserReception.claimName = function ( n ) {
	...
	}, window.NewUserReception.logStep = function ( n ) {window.HabboTracking.track ( "nux", "log", n )}, window.NewUserReception.saveOutfit = function ( n, e ) {
	...
	function n () {
		window._sp_video = new window.SPONSORPAY.Video.Iframe ( {
			...
			greyout: !1,
			callback_on_start: function ( n ) {window.HabboFlashClient.flashInterface.sponsorPayOnStart ( n.icon_small )},
			callback_no_offers: function () {window.HabboFlashClient.flashInterface.sponsorPayNoOffers ()},
			callback_on_close: function () {window.HabboFlashClient.flashInterface.sponsorPayOnClose ()},
			callback_on_conversion: function () {window.HabboFlashClient.flashInterface.sponsorPayOnConversion ()}
		} );
		try {
	...
	window.SponsorPay = {
		...
		backgroundLoad: function () {window._sp_video.backgroundLoad ()},
		showVideo: function () {window._sp_video.showVideo ()}
	}
} (), function () {
	"use strict";
	function n () {
	...
	function e ( n ) {window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignsReady ( n && n.length > 0 ? n.length.toString () : "0" )}

	function a () {window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignOpen ()}

	function o () {window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignClose ()}

	function t () {window.HabboFlashClient.flashInterface.supersonicAdsOnCampaignCompleted ()}
	...
	"use strict";
	window.TargetedWebOffer = {}, window.TargetedWebOffer.checkOffer = function () {console.log ( "Checking for offer..." ), window.HabboShopApi.checkOffer ( function ( n, e ) {return n ? void window.HabboFlashClient.flashInterface.targetedWebOfferCheckFailed () : void window.HabboFlashClient.flashInterface.targetedWebOfferCheckResponse ( e )} )}
} ();